...
改了组件 / API / 类型,看这里把三处 AI 数据源(website docs / llms.txt / skill)同步过去,别让下游 AI 读到漂移的用法。

- **API / props / 类型全量** → 文档站 + 远程 llms.txt(按需 fetch,**不在本仓正文镜像**,别凭记忆猜 API):
  - 文档站:https://unif-design.github.io/react-native-hms-scan/
...
- **website docs(`website/docs/`)是 llms.txt 的唯一来源** —— 改了组件 / API / 类型,**同步改 `website/docs/`** 再重生成 llms,否则 AI 读到的会过时(`website/scripts/build-llms.js`)。
- **改 API 也要同步消费侧 skill** —— skill `unif-hms-scan`(`unif-design/skills` 仓 `skills/unif-hms-scan/SKILL.md`):**手写部分**(快速开始示例 / 关键坑 / `assets/` 模板)手动改;**全量 props** 已路由 llms.txt,随 docs 自动跟随。
- **作为消费者接入** → Agent Skill `unif-hms-scan`(`unif` 插件),装:`/plugin marketplace add unif-design/skills` → `/plugin install unif@unif-skills`。经核实的 API / 坑 / 原生配置,是接入侧首选入口。
- CI / 发版 / 依赖管理 / branch protection 的配置与排查 SOP,集中在 org 共享文档 → https://github.com/unif-design/.github/blob/main/AUTOMATION.md(CI / release / native-lint 约定亦见 `README` + `.github/`)。
