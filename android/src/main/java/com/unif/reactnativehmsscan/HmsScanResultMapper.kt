...
internal object HmsScanResultMapper {
  ...
  private fun toJsonObject(scan: HmsScan, value: String): JSONObject {
    val obj = JSONObject()
    obj.put("value", value)
  ...
  fun mapScanType(scanType: Int): String = when (scanType) {
  ...
  private fun mapFormatString(format: String): Int? = when (format) {
   ...
   */
  private fun mapContentType(form: Int): String = when (form) {
