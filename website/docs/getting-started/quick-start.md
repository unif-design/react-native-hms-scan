...
若传 `autoConfirm` **且传了 `onConfirm`**,解析成功后会跳过结果卡、调用 `onConfirm` 并进入暂停的 `done` 终态,不会自动重扫；未传 `onConfirm` 会回退显示结果卡。**只有 `onConfirm` 正常返回才会进 `done`** —— 它和 `resolveProduct` 在同一个 `try` 里,同步抛错会被收成未识别的 fail 重扫层。权限 helper 或相机 view 的 fatal error 会进入可重试的 `error`;`E_NO_RESULT` 仅作为 soft error 上报，不打断当前扫码态。
