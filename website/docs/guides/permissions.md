...
`<Scanner>` 自动请求权限：未获授权进入 `denied` 设置遮罩；用户从系统设置返回 App 后会自动重新查询。权限 helper reject 会进入可重试的 `error`,并由 `onScanError` 上报普通 `{ code, message }`（不是 `HmsScanError`）。相机 view error 分三路:`E_NO_RESULT` 是 soft error,只上报、不离开扫码态;`E_NO_CAMERA_PERMISSION` 进入 `denied` 并卸载相机 view;`E_CAMERA_INIT` 等其他 fatal error 进入 `error`。
:::
